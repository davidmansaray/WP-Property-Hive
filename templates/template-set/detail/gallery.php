...
$show_floor_panel    = $is_preview && ! $is_editor_preview && $has_floor_map;
$show_tour_panel     = $is_preview && ! $is_editor_preview && $has_virtual_tour;
$show_map_panel      = ! $is_editor_preview && ! $use_cinema_controls && $location;
$floorplan_url       = $has_floor_map ? $floorplan_url : '';
...
<div class="images ph-template-gallery ph-template-gallery-<?php echo esc_attr( sanitize_html_class( $template ) ); ?> ph-template-gallery-<?php echo esc_attr( sanitize_html_class( $public_template ) ); ?> ph-gallery-variant-<?php echo esc_attr( sanitize_html_class( $gallery_layout ) ); ?>" data-ph-template-gallery data-ph-gallery-current-variant="<?php echo esc_attr( $gallery_layout ); ?>">
	...
	<figure class="ph-template-gallery-hero">
		...
		<?php else : ?>
			...
			<div class="ph-template-gallery-tabs" role="tablist" aria-label="<?php esc_attr_e( 'Gallery views', 'propertyhive' ); ?>"><button type="button" class="is-active" data-ph-gallery-tab="photos" aria-selected="true"><?php esc_html_e( 'Photos', 'propertyhive' ); ?></button><?php if ( $floorplan_url ) : ?><a href="<?php echo esc_url( $floorplan_url ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Floor map', 'propertyhive' ); ?></a><?php elseif ( $show_floor_panel ) : ?><button type="button" data-ph-gallery-tab="floorplan" aria-selected="false"><?php esc_html_e( 'Floor map', 'propertyhive' ); ?></button><?php endif; ?><?php if ( $is_editor_preview && ! empty( $virtual_tours[0]['url'] ) ) : ?><a href="<?php echo esc_url( $virtual_tours[0]['url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Virtual tour', 'propertyhive' ); ?></a><?php elseif ( $show_tour_panel ) : ?><button type="button" data-ph-gallery-tab="virtual-tour" aria-selected="false"><?php esc_html_e( 'Virtual tour', 'propertyhive' ); ?></button><?php endif; ?><?php if ( $is_editor_preview && $location_map_available ) : ?><a href="#ph-template-detail-location-map"><?php esc_html_e( 'Location', 'propertyhive' ); ?></a><?php elseif ( $show_map_panel ) : ?><button type="button" data-ph-gallery-tab="map" aria-selected="false"><?php esc_html_e( 'Location', 'propertyhive' ); ?></button><?php endif; ?></div>
		<?php endif; ?>
